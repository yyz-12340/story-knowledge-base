...
export default function Browse({ data }) {
  ...
  const [readingIdx, setReadingIdx] = useState(null); // 正在阅读的索引
  ...
  const closeReader = () => setReadingIdx(null);

  const goToNext = () => {
    if (readingIdx !== null && readingIdx < results.length - 1) {
      setReadingIdx(readingIdx + 1);
    }
  };

  const goToPrev = () => {
    if (readingIdx !== null && readingIdx > 0) {
      setReadingIdx(readingIdx - 1);
    }
  };
  ...
  return (
    <div className="browse-layout">
      ...
      <section>
        ...
        {results.slice(0, limit).map((a, idx) => (
          <button key={a.id} className="answer-card" onClick={() => openReader(idx)}>
            <h4>{a.question}</h4>
      ...
      {/* 阅读器 */}
      {readingIdx !== null && results[readingIdx] && (
        <Reader
          item={results[readingIdx]}
          contentMap={contentMap}
          onClose={closeReader}
          prevItem={readingIdx > 0 ? results[readingIdx - 1] : null}
          nextItem={readingIdx < results.length - 1 ? results[readingIdx + 1] : null}
          onNextItem={goToNext}
          onPrevItem={goToPrev}
        />
      )}
